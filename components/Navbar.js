    ...
    return (
        <nav className='bg-gray-900 shadow-lg shadow-white text-white flex justify-between px-4 items-center h-16'>
            <Link href={"/"} className='logo font-bold text-lg flex justify-center items-center gap-2'>
                <img className='rounded-full' width={44} 
            ...
            <div className='relative'>
                {session && <><button onClick={()=>setshowdropdown(!showdropdown)} onBlur={()=> {setTimeout(()=>{setshowdropdown(false)},300);}} id="dropdownDefaultButton" data-dropdown-toggle="dropdown" className="text-white mx-4 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">Welcome {session.user.email} <svg className="w-2.5 h-2.5 ms-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                    ...
                    <div id="dropdown" className={`z-10 ${showdropdown?"":"hidden"} absolute left-[160px] bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`}>
                ...
                {session && <Link href={'/login'}><button type="button" className="text-white bg-gradient-to-br from-purple-600
     to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none
      ...
      text-sm px-5 py-2.5 text-center me-2 mb-2" onClick={() => { signOut() }}>LogOut</button></Link>}
                {!session &&
                    <Link href={`${process.env.NEXT_PUBLIC_URL}/login`}><button type="button" className="text-white bg-gradient-to-br from-purple-600
     to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none
      focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg 
